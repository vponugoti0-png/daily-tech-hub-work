...
test.describe("SQL local practice lab + exercise path", () => {
  ...
  test("does not add a top-level Lab nav; Python ETL builder stays copy-only", async ({ page }) => {
    await page.goto("/");
    ...
    await expect(page.getByText(/Coming soon/i)).toHaveCount(0);
    await expect(tryit.getByRole("link", { name: /Run in local lab/i })).toHaveCount(0);
