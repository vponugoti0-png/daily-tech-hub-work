...
Practical notes for Frontend and QA. Implementation lives in `src/lib/auth/*` and `src/app/api/progress/route.ts`. Wave B backends (Practice Engine, Git VM, AI runtime) reuse this session + CSRF model: [`docs/wave-b-backend-ac.md`](./wave-b-backend-ac.md).
