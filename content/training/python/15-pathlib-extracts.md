...
description: "Treat incoming files as a landing contract: Path, glob, suffix, and stem. Do not concatenate cwd strings. Practice in the local lab (seeded VFS)."
level: beginner
order: -5
durationMinutes: 25
topics: [python]
...
cheatSheet:
  - label: "Landing glob"
    ...
    note: "Path / part is the join. glob is an allow-list. The local lab seeds /data/landing/orders."
  - label: "Stem contract"
...
**Edit and Run** in the local practice lab — it seeds `/data/landing/orders` so glob / stem samples work.
