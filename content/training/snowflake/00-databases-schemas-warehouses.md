...
cheatSheet:
  ...
  - label: "Warehouse parameters (comment as you go)"
    ...
    note: "Warehouses are compute, not storage. Auto-suspend so idle credits stop."
  - label: "Lab object map"
    code: "SELECT database, schema, object_name, object_type\nFROM sf_account_objects\nORDER BY database, schema, object_name;"
    note: "Run this SHOW-shaped inventory in the local practice lab — not a live account."
quiz:
